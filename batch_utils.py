...
class PairedImagePatchBatchLoader(BatchLoader):
    def create_dataset_from_generator(self):
        dataset = tf.data.Dataset.from_tensor_slices(self.PATHS)
        dataset = dataset.map(
            lambda x: tf.py_function(self.parse_image_pair, [x], [tf.float32, tf.float32]),
            num_parallel_calls=self.config.n_threads,
        )
        return dataset.map(self.set_shapes, num_parallel_calls=self.config.n_threads)

    def set_shapes(self, img, lab):
        img.set_shape([self.image_size, self.image_size, self.input_channels])
        lab.set_shape([self.image_size, self.image_size, self.label_channels])
        return img, lab

    def parse_image_pair(self, path):
        target_path = path.numpy().decode("UTF-8")
        input_path = self.config.convert_inp_path_from_target(target_path)

        img = cv2.imread(input_path, cv2.IMREAD_COLOR)
        lab = cv2.imread(target_path, cv2.IMREAD_COLOR)
        if img is None:
        ...
        if lab.shape[:2] != (self.image_size, self.image_size):
            lab = cv2.resize(lab, (self.image_size, self.image_size), interpolation=cv2.INTER_AREA)

        if img.shape[-1] > self.input_channels:
            img = img[:, :, :self.input_channels]
        elif img.shape[-1] < self.input_channels:
            img = np.repeat(img[:, :, :1], self.input_channels, axis=-1)

        if self.config.data_inpnorm == 'norm_by_mean_std':
            img = (img - np.mean(img)) / (np.std(img) + 1e-5)
        elif self.config.data_inpnorm == 'norm_by_specified_value':
            img = img / 255.0

        return img.astype(np.float32), lab.astype(np.float32)
