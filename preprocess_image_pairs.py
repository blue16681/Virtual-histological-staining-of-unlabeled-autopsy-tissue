...
def save_pair_patches(input_img, target_img, sample_key, split, out_root, patch_size, white_threshold, min_tissue):
    ...
    return saved, skipped_white


def main():
    parser = argparse.ArgumentParser(
        description="Rigidly register image pairs, tile 256x256 patches, remove white patches, and split train/test."
    )
    ...
    for idx, (input_path, target_path, key) in enumerate(pairs, 1):
        ...
        saved, skipped_white = save_pair_patches(
            aligned, fixed, key, split, out_root, args.patch_size,
            args.white_threshold, args.min_tissue,
        )
        print(f"  registration_ok={ok} ecc={ecc:.4f} saved={saved} skipped_white={skipped_white}")
